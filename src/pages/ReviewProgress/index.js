...
export default function ReviewProgress() {
  ...
  const handleSubmit = async (event) => {
    ...
    try{
        const respostaCodigo = await fetch (`http://127.0.0.1:8000/api/solicitacoes/busca-por-token/${codigoUnico}`);

        if( !respostaCodigo.ok ) {
            
        }

        const dadoRecebido = await respostaCodigo.json();
        ...
        if ( dadoRecebido.error ) {
            ...
            navigate(`/visualizar-solicitacao/${dadoRecebido.token_solicitacao}`)

            console.log(`/visualizar-solicitacao/${dadoRecebido.token_solicitacao}`)
