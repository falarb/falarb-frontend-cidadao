...
export default function Step005({ solicitacao, setSolicitacao, setStep }) {
  ...
  const criarsolicitação = async () => {
    try {
      ...
      const isInReboucas = await verificarMunicipio(
      ...
      if (!isInReboucas) {
        setMessageModal({
          ...
          descricao:
            "A localização selecionada não está dentro dos limites do município de Rebouças. Por favor, escolha uma localização válida dentro do município para prosseguir com a solicitação.",
        });
      ...
      if (error?.response?.data?.tipo === "texto ofensivo") {
        setMessageModal({
          ...
          descricao:
            "Sua solicitação contém palavras que não são permitidas em nossa plataforma. Por favor, revise o texto e tente novamente sem utilizar termos ofensivos.",
        });
  ...
  return (
    <>
      ...
      <div className="container-step-5">
        ...
        <BtnPrimary
          title={
            localizacaoPermitida
              ? "Ir para sua localização atual no mapa."
              : "Permitir que o navegador detecte sua localização atual."
          }
        ...
        <BtnPrimary
          title="Criar a solicitação após selecionar a localização correta no mapa."
        ...
        <BtnSecundary
          title="Voltar para a etapa anterior da solicitação."
        ...
        <BtnSecundary
          title="Cancelar a solicitação atual e retornar à tela inicial."
        ...
        {modalErroAberto && (
          <Modal
            ...
            description={
              messageModal?.descricao ||
              "Permita acesso à sua localização ou marque uma localização no mapa."
            }
