...
export default function Step004({
  ...
  useEffect(() => {
    ...
    async function buscaDados() {
      ...
      try {
        const listarCategorias = await axiosInstance.get(
          "/categorias?ordenar_por=nome&ordenar_direcao=asc"
        );
        setCategorias(listarCategorias?.data?.dados || []);

        const listarComunidades = await axiosInstance.get(
    ...
    <>
      {carregando ? (
        ...
        <div className="container-step-4">
          ...
          <h2>
            Tudo certo e verificado, você já pode começar sua solicitação.
          </h2>

          <small>
            É extremamente importante que você siga todos os passos, inserir
            todos os campos abaixo solicitados e ao final clicar em “Enviar
            solicitação”
          </small>
          ...
          <AutoCompleteCustom
            ...
            required
            options={comunidades}
            title="Selecione a comunidade relacionada à sua solicitação."
            onChange={(newValue) => {
              setSolicitacao((prev) => ({
                ...prev,
          ...
          <AutoCompleteCustom
            name="id_categoria"
            options={categorias}
            title="Selecione a categoria que melhor descreve o tipo da sua solicitação."
            onChange={(newValue) => {
              setSolicitacao((prev) => ({
                ...prev,
          ...
          <div style={{ position: "relative" }}>
            <TextArea
              ...
              title="Explique claramente o que você deseja solicitar. Seja o mais detalhado possível (máx. 256 caracteres)."
              value={solicitacao?.descricao}
              onChange={handleChange}
              maxLength={256}
            />
            <span
              className="contador-letras"
              style={{
                color: solicitacao?.descricao?.length > 246 ? "red" : "#888",
              }}
            >
          ...
          <BtnPrimary
            title="Avança para o próximo passo após preencher todos os campos obrigatórios."
            onClick={() => {
              if (
